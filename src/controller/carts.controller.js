...
export default class CartController {
  ...
  getOne = async (req, res, next) => {
    try {
      ...
      if (cid.length != 24)
        CustomError.createError({
          statusCode: 400,
          name: "Error leyendo la base de datos",
          cause: generateErrorInfo.getId(cid),
    ...
    try {
      ...
      if (!valid)
        CustomError.createError({
          ...
          cause: generateErrorInfo.getEmptyDatabase(),
          message: "There was no product that could be purchased",
          code: 4,
        }); //return res.send({status: 400, message: "You need to have products you can buy"});
  ...
  putProduct = async (req, res) => {
    try {
      ...
      CustomError.createError({
        statusCode: 404,
        name: "Product doesnt exist in cart",
        cause: generateErrorInfo.idNotFound(),
        code: 2,
      });
    } catch (error) {
